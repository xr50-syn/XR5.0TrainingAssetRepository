...
namespace XR50TrainingAssetRepo.Services
{
    ...
    public class MaterialService : IMaterialService
    {
        ...
        public async Task<Material> CreateMaterialAsyncComplete(Material material)
        {
            ...
            // Use explicit transaction to ensure proper rollback on failure
            ...
            try
            {
                ...
                context.Materials.Add(material);
                await context.SaveChangesAsync(); // Save to get the material ID

                _logger.LogInformation("Created material with ID: {Id}", material.Id);

                // Now handle child entities that need the MaterialId
                switch (material)
                {
                    case WorkflowMaterial workflow:
                        ...
                        if (workflow.WorkflowSteps?.Any() == true)
                        {
                            ...
                            foreach (var step in workflow.WorkflowSteps)
                            {
                                ...
                                // Create a completely new WorkflowStep to avoid any potential ID conflicts
                                var newStep = new WorkflowStep
                                {
                                    Title = step.Title,
                                    Content = step.Content
                                    // Id is not set - will be auto-generated by database
                                };
                                
                                // Set the shadow property for the foreign key
        ...
        // Add this method signature to the IMaterialService interface:


        // Add this method implementation to your MaterialService class
